...
def minimum_distance str
  locations = str.split(' ')[1..-1].map { |e| e.to_i }
  Range.new(*locations.minmax).each.with_object([]) do |house, a|
    a << locations.map { |loc| (loc - house).abs }.reduce(&:+)
  end.min
end

$stdin.read.split("\n").each { |line| puts minimum_distance(line) }