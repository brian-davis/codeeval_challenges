...
def matrix_rotation str
  elements    = str.split ' '
  side_length = Math.sqrt elements.size.to_i
  original    = elements.each_slice(side_length).to_a
  rotated     = original.transpose.map { |row| row.reverse  }
  serialized  = rotated.flatten.join ' '
end

$stdin.read.split("\n").each { |line| puts matrix_rotation(line) }
